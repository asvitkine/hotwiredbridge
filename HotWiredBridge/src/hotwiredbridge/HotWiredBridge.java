...
public class HotWiredBridge implements WiredEventHandler {
	...
	public void handleTransaction(Transaction t) throws IOException {
		...
		case Transaction.ID_MODIFYUSER: {
			...
			AccountPrivileges wiredPrivs = PrivsMapper.convertHotlineToWiredPrivs(privs);
			if (t.getId() == Transaction.ID_CREATEUSER) {
				client.createUser(login, password, "", wiredPrivs);
			} else {
				// TODO: Might be better to preserve old Wired values rather than resetting them.
				client.editUser(login, password, "", wiredPrivs);
			}
			break;
		}
				...
				for (Transaction t : pendingTransactions) {
					if (t.getId() == Transaction.ID_OPENUSER) {
						...
						if (login.equals(userAccountEvent.getName())) {
							...
							reply.addObject(TransactionObject.PRIVS, PrivsMapper.convertWiredPrivsToHotlinePrivs(userAccountEvent.getPriveleges()));
